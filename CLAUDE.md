...
- **Homepage content is admin-editable, not hardcoded.** `Site` (one lazily-created `default` row) + `SiteContent` (one JSON blob per section, unique on `siteId+section`). `src/lib/siteContent.js` holds each section's zod schema *and* its generic-placeholder default; `GET /api/site-content` returns stored data merged over defaults, so an unsaved section still renders. `PUT /:section` needs a full, valid section body (colors must be 6-digit hex); `DELETE /:section` resets to defaults; `POST /upload` stores an image and returns its URL. Adding a field means updating the zod schema + default here, the render component in `frontend/src/components/home/`, and the field config in `admin/src/pages/siteContent/SiteContent.jsx`. Icon names are validated only as strings; the allowed set lives in `frontend/src/lib/homeIcons.js` and `ICON_OPTIONS` in the admin page (keep in sync). The homepage fetches with `cache: "no-store"`. Multi-complex later = more `Site` rows keyed by request domain.
