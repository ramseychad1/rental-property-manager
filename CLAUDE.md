...
- Removed on purpose (no backend ever existed): the admin's Blogs and Error Logs screens. Also not implemented: Google OAuth *login* (Google OAuth is used only to connect Gmail for sending), BullMQ/Redis job queue, multi-complex tenancy (see "Repository state" above).
