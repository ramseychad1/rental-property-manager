...
export default function ServicesList({ services }) {
  ...
  return (
    ...
    >
      {services.length === 0 && (
        <p className="py-10 text-center text-[var(--color-muted-foreground)]">No services are listed right now.</p>
      )}
      {services.map((item, index) => {
        const service = { ...item, id: index };
        return (
        <article
          key={service.id}
        ...
        >
          <div className="relative aspect-[4/3] sm:aspect-auto bg-[var(--color-muted)]">
            {service.image ? (
              <Image
                src={service.image}
                alt={service.title}
              ...
              <div className="flex h-full min-h-[120px] items-center justify-center text-[var(--color-border)]">
          ...
          <div className="p-6">
            ...
            <p className="mt-3 text-sm leading-relaxed text-[var(--color-muted-foreground)]">
              {expanded[service.id] && service.description
                ? service.description
            ...
            <div className="mt-4 text-sm text-[var(--color-muted-foreground)]">
              {formatCurrency(service.price)}{service.priceNote ? ` (${service.priceNote})` : ""}
            </div>
          </div>
        </article>
        );
