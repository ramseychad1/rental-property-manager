  ...
  return (
    <section className="relative xl:h-[676px] lg:py-12 py-16 overflow-hidden" data-testid="home-hero">
      {heroContent.bgImage ? (
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-teal-deep)]" />
      )}
      <div className="absolute inset-0 bg-black" style={{ opacity: (heroContent.overlayOpacity ?? 40) / 100 }} />

      <div className="relative mx-auto max-w-7xl px-5 h-full items-start lg:grid lg:grid-cols-[1fr_380px] gap-10">
        {/* Left copy */}
        <div className={`mb-12 transition-opacity duration-400 ${isTransitioning ? "opacity-0" : "opacity-100 animate-fade-up"}`}
          style={{ color: heroContent.textColor }}>
          <p className="font-bold tracking-[0.3em] text-xs sm:text-sm uppercase mb-4"
            style={{ color: heroContent.eyebrowColor }}>
            {heroContent.eyebrow}
          </p>
          ...
          <p className="font-script text-4xl sm:text-5xl mt-6" style={{ color: heroContent.scriptColor }}>{heroContent.script}</p>
          <p className="mt-4 max-w-xl text-base sm:text-lg">{heroContent.body}</p>

          {/* Highlights */}
          <div className="mt-10 max-w-3xl">
            <ul className="flex items-center gap-0 sm:gap-8 flex-wrap justify-between md:justify-start md:flex-nowrap md:gap-18">
              {heroContent.highlights.map(({ label, icon }, i) => {
                ...
                <li key={i} className="flex flex-col items-center text-center">
                  <span className="flex h-8 sm:h-12 w-8 sm:w-12 items-center justify-center rounded-full bg-white/12 backdrop-blur-sm">
                    <Icon className="sm:h-6 h-4 sm:w-6 w-4" />
                  </span>
