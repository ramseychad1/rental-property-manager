...
- Not implemented: blog CMS, things-to-do CMS, error-log endpoints (the admin panel's `blogsApi`, `thingsToDoApi`, `errorLogsApi` will all 404 against this backend). Also not implemented: Google OAuth *login* (Google OAuth is used only to connect Gmail for sending), BullMQ/Redis job queue, multi-complex tenancy (see "Repository state" above).
