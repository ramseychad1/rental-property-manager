...
**Product intent:** this app is sold as a subscription to property owners within a condo complex; selling to another complex is the next step. Multi-tenancy is being built in stages. **Done:** per-owner scoping within one complex (`SuperAdmin` + `Owner` roles, `Property.ownerId`, see the Auth bullet in the backend section) and admin-editable homepage content. **Not done:** multi-complex (multiple `Site` rows resolved by request domain) and a complex-level `SiteAdmin` role — add those when a second complex is real.
...
- SuperAdmin: `admin@rentalpropertymanager.com` / `Admin123!` (this is the platform operator; create Owners from the admin panel's Users page)
...
- Routes live under `src/app/` (App Router — one folder per route: `properties/[id]`, `checkout`, `bookings`, `things-to-do/[id]`, `login`, `signup`, `forgot-password`, `reset-password`, `settings`, etc.).
...
- TipTap (`@tiptap/react`, `@tiptap/starter-kit`) is still a dependency from the removed Blogs feature and is currently unused.
...
- Auth is a JWT in an httpOnly cookie (`src/lib/jwt.js`, `src/middleware/auth.js`), read on every request by an always-on `attachUser` middleware so both public and admin routes can check `req.user`. `requireAuth`/`requireStaff`/`requireSuperAdmin` gate individual routes.
  - **Roles:** `Guest` (public site), `Owner` (manages only properties with `ownerId === their id`), `SuperAdmin` (everything). `src/lib/access.js` holds the scoping helpers (`propertyScope`, `bookingScope`, `findManagedProperty`) — use them for any new staff-facing query rather than re-deriving the rule; non-owned ids return 404, not 403. Properties with `ownerId = null` are unassigned and visible only to SuperAdmin. Site Content and the Users endpoints are SuperAdmin-only.
  - **Managing view vs public view:** `GET /property` returns only active properties to everyone *unless* the caller is staff and sends the admin panel's `X-Client: rental-property-manager-admin-panel` header, which switches to all-statuses scoped to what they may manage. This keeps an Owner who is browsing the public site from seeing a scoped list. The header only picks the view; authorization is always the role.
  - **Users are created by a SuperAdmin, not by email:** `POST /user` returns a one-time temporary password plus login URL (the request's `Origin`), which the admin UI shows with a "Copy all" button for the SuperAdmin to send manually. Accounts created or reset this way have `mustChangePassword`, and `enforcePasswordChange` (`middleware/auth.js`) 403s everything except changing the password (and public GETs) until it's done. `isActive = false` accounts can't sign in and existing sessions stop working.
  - The seed creates the `SuperAdmin`. The `20260919230000_roles_and_ownership` migration renamed the old `Admin` enum value to `SuperAdmin` in place.
- Signup is OTP-gated (`src/lib/otp.js`): `POST /otp/send-otp` issues a 6-digit code tied to an email+purpose, `POST /auth/register` consumes it. Codes are emailed via the system Gmail sender (see the email bullet below); with none connected they're printed to the backend's console instead. `POST /auth/register`'s response shape intentionally deviates from the rest of the API (`{message, user}`, not `{data}`) — that's not a bug, the frontend's `AuthContext.signup()` reads `data.user` directly.
...
- **Season date ranges are yearly-recurring `"MM-DD"` strings, not dates** (`dateRanges: [{startDate: "06-01", endDate: "09-01"}]`); an end before the start wraps over New Year. Matching lives in `backend/src/lib/seasonRange.js` and is duplicated in `frontend/src/lib/seasonRange.js` / `admin/src/lib/seasonRange.js` (same rule, keep in sync). Legacy `YYYY-MM-DD` values are tolerated on read/write by dropping the year; migration `20260920120000_season_month_day` rewrote existing rows.
...
- **Public-site content is admin-editable, not hardcoded.** `Site` (one lazily-created `default` row) + `SiteContent` (one JSON blob per section, unique on `siteId+section`). `src/lib/siteContent.js` holds each section's zod schema *and* its generic-placeholder default; `GET /api/site-content` returns stored data merged over defaults, so an unsaved section still renders. `PUT /:section` needs a full, valid section body (colors must be 6-digit hex, links `http(s)://`); `DELETE /:section` resets to defaults; `POST /upload` stores an image and returns its URL. Sections: homepage (`hero`, `featureBar`, `lifestyle`, `nearby`, `testimonials`, `trustBand`), site-wide (`brand` = name/tagline/phone/email/address/social links used by header, footer and Contact page; `seo` = default title/description/keywords/share image), and pages (`propertiesPage`, `servicesPage`, `contactPage`, `privacyPage`, `refundPage`). Adding a field means updating the zod schema + default here, the render component in `frontend/src/`, and the field config in `admin/src/pages/siteContent/SiteContent.jsx` (a `PAGES` list = left menu; a page with several sections gets tabs; field types include `heading`/`note` for grouping). Frontend readers use `src/lib/getSiteContent.js` (`getSiteContent`/`getBrand`, React-`cache`d per request) and must tolerate a missing section. **The root layout is `force-dynamic`** because header/footer/metadata read this content per request. Empty phone/email/address/social/map values hide their UI. Icon names are validated only as strings; the allowed set lives in `frontend/src/lib/homeIcons.js` and `ICON_OPTIONS` in the admin page (keep in sync). Multi-complex later = more `Site` rows keyed by request domain. `thingsPage` holds the Things To Do banner/messages. Still hardcoded: the `/rate` page and legacy `lib/mockData.js` (Florida Keys leftovers, not linked from the main nav).
- **Email goes out through connected Gmail accounts, not a mail server** (setup + Google verification notes: `docs/gmail-setup.md`). `EmailConnection` holds a user's encrypted (AES-256-GCM, `EMAIL_TOKEN_KEY`) refresh token; `src/lib/mailer.js` `sendMail()` picks the property owner's connection, else the SuperAdmin's `isSystem` connection, else just logs to the console (so dev and unconfigured deploys work). It never throws into a request, records every attempt in `EmailLog` (no bodies), and on a Google `invalid_grant`/401 marks the connection `needs_reconnect` and retries via the system sender. Callers use `src/lib/notifications.js` (`void notifyBookingCreated(...)` after responding), templates are in `src/lib/emailTemplates.js` (HTML-escape everything, format stay dates in UTC). OTP codes, password resets and the contact form always use the system sender. The OAuth `state` is a signed 10-minute JWT carrying the user id and the admin origin (validated against `CLIENT_URLS`), so the callback doesn't depend on the session cookie. `GOOGLE_OAUTH_BASE`/`GMAIL_API_BASE` can point at a local fake for testing. Owner-facing UI: admin Settings -> Email card.
- **Things To Do** is a real Super-Admin-managed table (`ThingToDo`: name, description, free-text `category` and `area`, address, map URL, one image, `active`/`inactive`), API at `/api/thingtodo` (public list = active only; the admin panel's `X-Client` header gives Super Admins every status). Only name and category are required. The public page groups by category (tabs) then area. **Blogs were removed on purpose** (no backend ever existed and the owner doesn't want the feature) - don't resurrect the frontend/admin screens without a backend.
- Not implemented: error-log endpoints (the admin panel's `errorLogsApi` and its "Error Logs" menu item 404 against this backend). Also not implemented: Google OAuth *login* (Google OAuth is used only to connect Gmail for sending), BullMQ/Redis job queue, multi-complex tenancy (see "Repository state" above).
